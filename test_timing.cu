...
int main(int argc, char* argv[])
{
    ...
    save_file<double>("data/analytical.bin",(void*) h_analytical, NX*NY*NZ );
    ...
    snprintf(timing_filename, 256, "data/timing%ux%ux%u.dat", lattice[0], lattice[1], lattice[2]);
    ...
    for (unsigned threads = 32; threads <= 1024; threads += 32)
    {
        ...
        for (unsigned it = 0; it < iters; it++)
        {
            ...
            //save_file<cuDoubleComplex>("orginal3D.bin",(void*) h_psi, NXYZ);
            ...
            //if (it == 0) save_file<double>("data/vcoulomb.bin",(void*) h_psi, NXYZ);
