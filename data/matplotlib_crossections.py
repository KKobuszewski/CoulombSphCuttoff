...
def plot_crosssections():
    ...
    im = ax1.imshow(np.log10(resized_data[:,:,Nz2/2].T+1e-50), interpolation='bilinear', origin='lower',cmap=cm.rainbow, extent=(-Nx2/2.,Nx2/2., -Ny2/2.,Ny2/2.))
    ...
    im = ax2.imshow(np.log10(resized_data[:,Ny2/2,:].T+1e-50), interpolation='bilinear', origin='lower',cmap=cm.rainbow, extent=(-Nx2/2.,Nx2/2., -Nz2/2.,Nz2/2.))
    #cb = plt.colorbar(im, shrink=0.8, extend='both')
    #cb.set_label(r'$log_{10}\left(\vert\psi\vert^2a_{ho}^3\right)$')
    ...
    im = ax3.imshow(np.log10(resized_data[Nx2/2,:,:].T+1e-50), interpolation='bilinear', origin='lower',cmap=cm.rainbow, extent=(-Ny2/2.,Ny2/2., -Nz2/2.,Nz2/2.))
    cb = plt.colorbar(im, shrink=0.8, extend='both')
    cb.set_label(r'$log_{10}\left(\vert\psi\vert^2a_{ho}^3\right)$')
